	...
	return (
		...
		>
			...
			<div className="flex min-w-0 flex-1 flex-col gap-0.5">
				<p
					className={cn(
						"truncate font-semibold text-[15px] leading-snug",
						variant.title,
					)}
				>
					{event.title}
				...
				<p
					className={cn(
						"truncate text-[13px] tabular-nums leading-snug",
						variant.desc,
					)}
				>
					{event.desc}
	...
	return (
		...
		>
			<div className="flex shrink-0 items-baseline gap-1.5">
				<p className="font-semibold text-[15px] text-rose-500 leading-snug">
					{/* {date.toLocaleString("default", { weekday: "short" })} */}{" "}
					Projects
				</p>
				<p className="font-normal text-[22px] text-foreground tabular-nums leading-snug tracking-tight">
			...
			{visible.length > 0 ? (
				...
				<p className="mt-2 shrink-0 text-[13px] text-muted-foreground leading-snug">
					No projects pending
				</p>
			)}
			...
			{extra > 0 ? (
				<div className="mt-1.5 flex h-8 shrink-0 items-center justify-between gap-2 rounded-md bg-border/60 px-4.5">
					<p className="shrink-0 font-medium text-[13px] text-foreground leading-snug">
						+{extra} more
					</p>
					<p className="min-w-0 truncate text-right text-[13px] text-muted-foreground tabular-nums leading-snug">
